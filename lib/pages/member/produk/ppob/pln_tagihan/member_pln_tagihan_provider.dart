...
class MemberPlnTagihanState extends Equatable {
  // Product API
  ...
  MemberPlnTagihanState copyWith({
    // Product API
    ...
    return MemberPlnTagihanState(
      ...
      isNontaglist: isNontaglist ?? this.isNontaglist,
...
class MemberPlnTagihanProvider extends Cubit<MemberPlnTagihanState> {
  final ProdukService _produkService = ProdukService();

  MemberPlnTagihanProvider()
    : super(
        MemberPlnTagihanState(
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberPlnTagihanProvider, MemberPlnTagihanState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarPlnTagihanMember(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getPlnTagihanProducts();
      if (!result.status) {
        emit(
          state.copyWith(
      ...
      if (data != null) {
        emit(
          state.copyWith(
            ...
            products: [data.plnpasca, data.plnnontaglist],
          ),
        );
      } else {
        emit(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekPlnTagihan(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void setSelectedProduct(ProductModel product) {
    ...
    var namaProdukLower = product.namaproduk.toLowerCase();
    namaProdukLower = removeNonAlphanumeric(namaProdukLower).trim();
    if (namaProdukLower.contains('nontaglist')) {
      emit(state.copyWith(isNontaglist: true));
    } else {
      emit(state.copyWith(isNontaglist: false));
    }
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberPlnTagihanKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    >(
      context,
      title: 'Pilih Produk',
      subtitle: 'Pilih produk dari daftar berikut',
      bloc: this,
...
MemberPlnTagihanProvider getMemberPlnTagihanProvider(BuildContext context) =>
    context.read<MemberPlnTagihanProvider>();
