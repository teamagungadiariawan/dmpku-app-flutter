...
class MemberESamsatState extends Equatable {
  // Product API
  ...
  final FocusNode? noMesinFocusNode;
  final TextEditingController? noMesinController;
  final bool noMesinHasError;
  final String noMesinErrorMessage;

  // Single NIK
  final String nik;
  final FocusNode? nikFocusNode;
  final TextEditingController? nikController;
  final bool nikHasError;
  final String nikErrorMessage;
  ...
  const MemberESamsatState({
    ...
    // Single No Mesin
    this.noMesin = '',
    this.noMesinFocusNode,
    this.noMesinController,
    this.noMesinHasError = false,
    this.noMesinErrorMessage = '',

    // Single NIK
    this.nik = '',
    this.nikFocusNode,
    this.nikController,
    this.nikHasError = false,
  ...
  MemberESamsatState copyWith({
    ...
    // Single No Mesin
    String? noMesin,
    FocusNode? noMesinFocusNode,
    TextEditingController? noMesinController,
    bool? noMesinHasError,
    String? noMesinErrorMessage,
    ...
    return MemberESamsatState(
      ...
      noMesinFocusNode: noMesinFocusNode ?? this.noMesinFocusNode,
      noMesinController: noMesinController ?? this.noMesinController,
      noMesinHasError: noMesinHasError ?? this.noMesinHasError,
      noMesinErrorMessage: noMesinErrorMessage ?? this.noMesinErrorMessage,

      // Single NIK
      nik: nik ?? this.nik,
      nikFocusNode: nikFocusNode ?? this.nikFocusNode,
      nikController: nikController ?? this.nikController,
      nikHasError: nikHasError ?? this.nikHasError,
      nikErrorMessage: nikErrorMessage ?? this.nikErrorMessage,
...
class MemberESamsatProvider extends Cubit<MemberESamsatState> {
  final ProdukService _produkService = ProdukService();

  MemberESamsatProvider()
    : super(
        MemberESamsatState(
          tujuanFocusNode: FocusNode(),
          tujuanController: TextEditingController(),
          searchProductController: TextEditingController(),
          noMesinFocusNode: FocusNode(),
          noMesinController: TextEditingController(),
          nikFocusNode: FocusNode(),
  ...
  Future<void> close() {
    ...
    state.noMesinFocusNode?.dispose();
    state.noMesinController?.dispose();
    state.nikFocusNode?.dispose();
    state.nikController?.dispose();
    return super.close();
  }
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberESamsatProvider, MemberESamsatState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarPkb(
        ...
        tujuantambahan: state.isJatim
            ? state.noMesin.trim() + "@" + state.nik.trim()
            : 'kosong',
      );
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getPkbProducts();
      if (!result.status) {
        emit(
          state.copyWith(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    if (state.isJatim) {
      var validNoMesin = validateNoMesin();
      if (!validNoMesin) return;

      var validNIK = validateNIK();
      if (!validNIK) return;
    }
    ...
    try {
      final result = await _produkService.cekPkb(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void setSelectedProduct(ProductModel product) {
    ...
    var namaProdukLower = product.kodeproduk.toLowerCase();
    namaProdukLower = removeNonAlphanumeric(namaProdukLower).trim();
    debugPrint('Nama Produk Lower: $namaProdukLower');
    // remove space and check if contains "jatim"
    if (namaProdukLower.contains('jawa_timur')) {
      emit(state.copyWith(isJatim: true));
    } else {
      emit(state.copyWith(isJatim: false));
    }
  }
  ...
  void setNoMesin(String value, {bool updateController = false}) {
    var val = value.trim();

    emit(state.copyWith(noMesin: val));
    if (updateController) _updateController(state.noMesinController, val);

    validateNoMesin();
  }

  void setNIK(String value, {bool updateController = false}) {
    var val = value.trim();

    emit(state.copyWith(nik: val));
    if (updateController) _updateController(state.nikController, val);
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberESamsatKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  bool validateNoMesin() {
    final error = _validateNoMesinValue(
      state.noMesin.trim(),
      state.selectedProduct,
    );
    ...
    emit(
      state.copyWith(
        noMesinHasError: error != null,
        noMesinErrorMessage: error ?? '',
      ),
    );
  ...
  bool validateNIK() {
    final error = _validateNIKValue(state.nik.trim(), state.selectedProduct);

    emit(
      state.copyWith(nikHasError: error != null, nikErrorMessage: error ?? ''),
    );
  ...
  String? _validateNoMesinValue(String tujuan, ProductModel product) {
    if (tujuan.isEmpty) {
      return 'No Mesin tidak boleh kosong';
    }

    // Validasi panjang
    if (tujuan.length < 8 || tujuan.length > 17) {
      return 'Panjang tujuan harus antara 8 hingga 17 karakter';
    }

    // Validasi tipe input
    if (!TipeInput.alphanumeric.isValid(tujuan)) {
      return TipeInput.alphanumeric.errorMessage;
    }

    return null;
  }

  String? _validateNIKValue(String tujuan, ProductModel product) {
    if (tujuan.isEmpty) {
      return 'Tujuan tidak boleh kosong';
    }
    ...
    // Validasi panjang
    if (tujuan.length != 16) {
      return 'Panjang tujuan harus 16 karakter';
    }
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    PilihProductPpobDialog.show<MemberESamsatProvider, MemberESamsatState>(
      context,
      title: 'Pilih E-Samsat',
      subtitle: 'Pilih produk E-Samsat dari daftar berikut',
      bloc: this,
...
MemberESamsatProvider getMemberESamsatProvider(BuildContext context) =>
    context.read<MemberESamsatProvider>();
