...
class MemberBpjsTknState extends Equatable {
  // Product API
...
class MemberBpjsTknProvider extends Cubit<MemberBpjsTknState> {
  final ProdukService _produkService = ProdukService();

  MemberBpjsTknProvider()
    : super(
        MemberBpjsTknState(
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberBpjsTknProvider, MemberBpjsTknState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarBpjsKetenagakerjaan(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getBpjsKetenagakerjaanProducts();
      if (!result.status) {
        emit(
          state.copyWith(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekBpjsKetenagakerjaan(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberBpjsTknKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    >(
      context,
      title: 'Pilih BPJS Ketenagakerjaan',
      subtitle: 'Pilih produk BPJS Ketenagakerjaan dari daftar berikut',
      bloc: this,
...
MemberBpjsTknProvider getMemberBpjsTknProvider(BuildContext context) =>
    context.read<MemberBpjsTknProvider>();
