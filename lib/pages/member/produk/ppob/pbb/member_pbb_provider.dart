...
class MemberPbbState extends Equatable {
  // Product API
...
class MemberPbbProvider extends Cubit<MemberPbbState> {
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberPbbProvider, MemberPbbState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarPbb(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getPbbProducts();
      if (!result.status) {
        emit(
          state.copyWith(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekPbb(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void setTahun(String value) {
    var val = value.trim();

    emit(state.copyWith(tahun: val));
  }
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberPbbKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    PilihProductPpobDialog.show<MemberPbbProvider, MemberPbbState>(
      context,
      title: 'Pilih PBB',
      subtitle: 'Pilih produk PBB dari daftar berikut',
      bloc: this,
...
MemberPbbProvider getMemberPbbProvider(BuildContext context) =>
    context.read<MemberPbbProvider>();
