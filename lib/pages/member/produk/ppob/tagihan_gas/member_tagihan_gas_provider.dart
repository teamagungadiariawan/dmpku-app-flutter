...
class MemberTagihanGasState extends Equatable {
  // Product API
  ...
  MemberTagihanGasState copyWith({
    // Product API
...
class MemberTagihanGasProvider extends Cubit<MemberTagihanGasState> {
  final ProdukService _produkService = ProdukService();

  MemberTagihanGasProvider()
    : super(
        MemberTagihanGasState(
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberTagihanGasProvider, MemberTagihanGasState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarTagihanGas(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getTagihanGasProducts();
      if (!result.status) {
        emit(
          state.copyWith(
      ...
      if (data != null) {
        emit(
          state.copyWith(
            ...
            products: [data.pertagaspasca, data.pgnpasca],
          ),
        );
      } else {
        emit(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekTagihanGas(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberTagihanGasKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    >(
      context,
      title: 'Pilih Tagihan Gas',
      subtitle: 'Pilih produk Tagihan Gas dari daftar berikut',
      bloc: this,
...
MemberTagihanGasProvider getMemberTagihanGasProvider(BuildContext context) =>
    context.read<MemberTagihanGasProvider>();
