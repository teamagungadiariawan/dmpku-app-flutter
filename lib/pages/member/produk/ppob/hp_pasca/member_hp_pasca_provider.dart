...
class MemberHpPascaState extends Equatable {
  // Product API
...
class MemberHpPascaProvider extends Cubit<MemberHpPascaState> {
  final ProdukService _produkService = ProdukService();

  MemberHpPascaProvider()
    : super(
        MemberHpPascaState(
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberHpPascaProvider, MemberHpPascaState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarHpPascaMember(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getHpPascaProducts();
      if (!result.status) {
        emit(
          state.copyWith(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekHpPasca(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberHpPascaKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
  ...
  void showPilihProdukDialog(BuildContext context) async {
    ...
    >(
      ...
      subtitle: 'Pilih produk operator dari daftar berikut',
      bloc: this,
...
MemberHpPascaProvider getMemberHpPascaProvider(BuildContext context) =>
    context.read<MemberHpPascaProvider>();
