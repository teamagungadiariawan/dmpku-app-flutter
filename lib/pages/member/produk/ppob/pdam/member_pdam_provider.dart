...
class MemberPdamState extends Equatable {
  // Product API
...
class MemberPdamProvider extends Cubit<MemberPdamState> {
  ...
  void konfirmasiTrx(BuildContext context) async {
    ...
    KonfirmasiPinDialog.show<MemberPdamProvider, MemberPdamState>(
      context,
  ...
  Future<void> _prosesKonfirmasi(BuildContext context, String pin) async {
    ...
    try {
      ...
      final result = await _produkService.bayarPdam(
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final result = await _produkService.getPdamProducts();
      if (!result.status) {
        emit(
          state.copyWith(
  ...
  Future<void> cekTagihan(BuildContext context) async {
    ...
    try {
      final result = await _produkService.cekPdam(
        kodeproduk: state.selectedProduct.kodeproduk,
        tujuan: state.tujuan,
      );
  ...
  void kelolaCekTagihan(BuildContext context, CekTagihanResponse response) {
    if (response.status) {
      if (response.dataSplit != null) {
        ...
        pushNamed(MemberPdamKonfirmasiTransaksiPage.routeName);
      }
    } else {
      List<KeyValue> dataTransaksi = response.dataSplit?.dataTransaksi ?? [];
      dataTransaksi = dataTransaksi
...
MemberPdamProvider getMemberPdamProvider(BuildContext context) =>
    context.read<MemberPdamProvider>();
