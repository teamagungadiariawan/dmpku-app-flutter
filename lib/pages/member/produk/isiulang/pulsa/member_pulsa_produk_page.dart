...
class MemberPulsaProdukPage extends StatefulWidget {
  static const routeName = '/member/produk/isiulang/pulsa/produk';

  const MemberPulsaProdukPage({super.key});

  @override
  State<MemberPulsaProdukPage> createState() => _MemberPulsaProdukPageState();
}

class _MemberPulsaProdukPageState extends State<MemberPulsaProdukPage> {
  final shakeKey = GlobalKey<ShakeErrorWidgetState>();

  void closePage() {
    getMemberPulsaProvider(context).resetProduct();
    pop();
  }
    ...
    return BlocBuilder<MemberPulsaProvider, MemberPulsaState>(
      ...
      builder: (context, state) {
        return SortFilterProduct(
          ...
          onSearchChanged: (val) {
            getMemberPulsaProvider(context).setSearchProduct(val);
          },
          onClearSearch: () {
            getMemberPulsaProvider(
              context,
          ...
          onSortSelected: (sortBy) {
            getMemberPulsaProvider(context).setSortProduct(sortBy);
          },
        );
      },
    );
        ...
        return RefreshableList(
          ...
          itemBuilder: (context, provider, index) {
            ...
            return CardProductPulsa(
              ...
              isPulsa: true,
              onPress: () {
                getMemberPulsaProvider(context).setSelectedProduct(product);
              },
            );
          },
