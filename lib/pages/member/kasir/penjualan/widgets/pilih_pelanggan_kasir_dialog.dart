...
class PilihPelangganKasirDialog extends StatefulWidget {
  const PilihPelangganKasirDialog({super.key});
  ...
  static void show(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => BlocProvider.value(
        value: provider,
        child: const PilihPelangganKasirDialog(),
      ),
    );
  }

  @override
  State<PilihPelangganKasirDialog> createState() =>
      _PilihPelangganKasirDialogState();
}

class _PilihPelangganKasirDialogState extends State<PilihPelangganKasirDialog> {
  ...
  List<PelangganModel> _filterList(List<PelangganModel> list) {
    if (_searchQuery.isEmpty) return list;
    return list.where((item) {
      final name = item.namapelanggan.toLowerCase();
      final phone = item.nohppelanggan.toLowerCase();
        ...
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(MdiIcons.accountGroup),
            ),
            const Gap(10),
            Expanded(
              child: Column(
            ...
            CustomButton(
              text: "Refresh",
              onPressed: () => _provider.fetchListPelanggan(),
              height: 28,
  ...
  Widget _buildList(BuildContext context) {
    return BlocBuilder<KasirProvider, KasirState>(
      builder: (context, state) {
        if (state.apiGetPelangganStatus.isLoading) {
          return const CardProviderListShimmer(itemCount: 5);
        }

        if (state.apiGetPelangganStatus.isFailure) {
          return Center(
            child: Column(
        ...
        final filteredList = _filterList(state.listPelanggan);
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final isSelected =
                state.selectedPelanggan?.idpelanggan == item.idpelanggan;
            ...
            return Card(
              ...
              child: InkWell(
                ...
                onTap: () {
                  _provider.onChangeSelectedPelanggan(item);
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            if (item.nohppelanggan.isNotEmpty)
                              Text(
                                item.nohppelanggan,
                            ...
                            if (item.alamatpelanggan.isNotEmpty)
                              Text(
                      ...
                      // Trailing icon
                      isSelected
                          ? Icon(MdiIcons.checkCircle, color: context.primary)
                          : Icon(
