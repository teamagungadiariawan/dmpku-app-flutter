...
class RiwayatListMutasiStok extends StatelessWidget {
  const RiwayatListMutasiStok({super.key});
      ...
      child: Column(
        children: [
          const RiwayatTableHeader(
            columns: [
              {'label': 'Waktu', 'width': 80.0, 'align': TextAlign.start},
              {'label': 'Keterangan', 'flex': true, 'align': TextAlign.start},
              {'label': 'Potongan', 'width': 60.0, 'align': TextAlign.end},
              {'label': 'Stok', 'width': 50.0, 'align': TextAlign.end},
            ],
          ),
          Expanded(
            child: BlocBuilder<MemberRiwayatProvider, MemberRiwayatState>(
              ...
              builder: (context, state) {
                return LoadMoreRefreshableList<MutasiSaldoModel>(
                  ...
                  isLoadingMore: state.apiFetchMutasiStokStatus.isLoading &&
                      state.pageMutasiStok > 1,
                  onLoadMore: () {
                    context.read<MemberRiwayatProvider>().nextPageMutasiStok();
                  },
                );
              },
            ),
