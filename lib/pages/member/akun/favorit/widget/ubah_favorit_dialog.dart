...
class UbahFavoritDialog extends StatefulWidget {
  final int idKategori;
  final FavoritModel favoritModel;

  const UbahFavoritDialog({
    super.key,
    required this.idKategori,
    required this.favoritModel,
  });

  static void show(
    BuildContext context, {
    required int idKategori,
    required FavoritModel favoritModel,
  }) {
    showModalBottomSheet(
      ...
      useSafeArea: true,
      builder: (_) =>
          UbahFavoritDialog(idKategori: idKategori, favoritModel: favoritModel),
    );
  }

  @override
  State<UbahFavoritDialog> createState() => _UbahFavoritDialogState();
}

class _UbahFavoritDialogState extends State<UbahFavoritDialog> {
  ...
  void initState() {
    ...
    _namaFavoritController = TextEditingController(
      text: widget.favoritModel.nama,
    );
    _nomorFavoritController = TextEditingController(
      text: widget.favoritModel.nomor,
    );
  }
  ...
  void simpanFavorit() async {
    if (validateUbahFavorit()) {
      // Simpan favorit ke database atau API di sini

      var suc = await getMemberFavoritProvider(context).editFavorit(
        context,
        kategori: kat,
        nama: _namaFavoritController.text.trim(),
        nomor: _nomorFavoritController.text.trim(),
        idfavorit: widget.favoritModel.idfavorit,
      );
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Ubah Tujuan Favorit Anda untuk kemudahan akses di masa mendatang.',
              ...
              builder: (context, state) {
                return Padding(
                  ...
                  child: CustomButton(
                    ...
                    isLoading: state.apiUbahFavoritStatus.isLoading,
                    text: "Simpan Favorit",
                    onPressed: () {
                      simpanFavorit();
                    },
