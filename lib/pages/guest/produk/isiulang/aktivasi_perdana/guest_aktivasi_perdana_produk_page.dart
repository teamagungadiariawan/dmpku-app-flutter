...
class GuestAktivasiPerdanaProdukPage extends StatefulWidget {
  static const routeName = '/guest/produk/isiulang/aktivasi-perdana/produk';

  const GuestAktivasiPerdanaProdukPage({super.key});

  @override
  State<GuestAktivasiPerdanaProdukPage> createState() =>
      _GuestAktivasiPerdanaProdukPageState();
}

class _GuestAktivasiPerdanaProdukPageState
    extends State<GuestAktivasiPerdanaProdukPage> {
  final shakeKey = GlobalKey<ShakeErrorWidgetState>();

  void closePage() {
    getAktivasiPerdanaProvider(context).resetProduct();
    pop();
  }
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: WillPopScope(
        ...
        child: Scaffold(
          appBar: CustomAppBar(
            title: getAktivasiPerdanaProvider(
              context,
          ...
          bottomNavigationBar:
              BlocBuilder<AktivasiPerdanaProvider, AktivasiPerdanaState>(
  ...
  Widget _buildPhoneNumberCard(BuildContext context) {
    return BlocBuilder<AktivasiPerdanaProvider, AktivasiPerdanaState>(
      buildWhen: (previous, current) =>
          previous.tujuan != current.tujuan ||
          previous.tujuanHasError != current.tujuanHasError,
      builder: (context, state) {
        return CardInputTujuan(
          ...
          onChanged: (value) {
            getAktivasiPerdanaProvider(context).setTujuan(value);
          },
          onClear: () {
            getAktivasiPerdanaProvider(
              context,
    ...
    return BlocBuilder<AktivasiPerdanaProvider, AktivasiPerdanaState>(
      ...
      builder: (context, state) {
        return SortFilterProduct(
          ...
          onSearchChanged: (val) {
            getAktivasiPerdanaProvider(context).setSearchProduct(val);
          },
          onClearSearch: () {
            getAktivasiPerdanaProvider(
              context,
          ...
          onSortSelected: (sortBy) {
            getAktivasiPerdanaProvider(context).setSortProduct(sortBy);
          },
        );
      },
    );
        ...
        return RefreshableList(
          ...
          itemBuilder: (context, provider, index) {
            ...
            return CardProductPulsa(
              ...
              isPulsa: true,
              onPress: () {
                getAktivasiPerdanaProvider(context).setSelectedProduct(product);
              },
            );
          },
